...
public class EndpointMapMetadata
{
    ...
    public EndpointMapMetadata WithGroup(string? group, bool throwIfNull = false)
    {
        ...
        Group = string.IsNullOrWhiteSpace(group) ? null : RoutePatternFactory.Parse(group);
    ...
    AutoWrapResultAttribute? _autoWarpResultMetadata = null;
    public EndpointMapMetadata WithAutoWarpResut(bool isAutoMap = true)
    {
        if ( isAutoMap )
            _metadata.Add(_autoWarpResultMetadata ??= new());
        else if ( _autoWarpResultMetadata != null )
            _metadata.Remove(_autoWarpResultMetadata);

        return this;
    }

    public EndpointMapMetadata WithAntiforgery(bool isAntiforgery = false)
    {
        if ( isAntiforgery )
            _metadata.Remove(AntiforgeryMetadata.ValidationNotRequired);
        else
            _metadata.Add(AntiforgeryMetadata.ValidationNotRequired);
