...
public class AutoMapEndPointSample : IAutoMapEndPoints
{
    ...
    /// <returns></returns>
    public IEnumerable<WeatherForecast> GetWeatherForecast(HttpContext context, [FromForm] TestParamDto? testId, [FromQuery] int? id2 = 2)
    {
        var cfgDic=_config.GetSection("EndPoints:WeatherForeCast").Get<Dictionary<string, object>>();
        ...
        var ex = new WebApiException(403, "你没有权限！")
        //可选：异常的明细信息。
        {
            Data = {
                ["ke1"]=id2,
...
public record TestParamDto(int? Id = 1);